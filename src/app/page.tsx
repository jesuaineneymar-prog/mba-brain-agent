...
function computeDashboard() {
  ...
  for (var i = 0; i < profiles.length; i++) {
    ...
    for (var k = msgs.length - 1; k >= 0; k--) {
      if (msgs[k].direction === 'outbound') { lastOut = msgs[k]; break; }
    }
    if (lastOut) {
      ...
      if (!hasReplyAfter && (now - sentTime) > threeDays) {
        var alreadyFU = false;
        for (var nn = 0; nn < msgs.length; nn++) {
          if (msgs[nn].direction === 'outbound' && msgs[nn].type === 'follow-up' && new Date(msgs[nn].sentAt).getTime() > sentTime) { alreadyFU = true; break; }
        }
        if (!alreadyFU) pendingFollowUps++;
      }
    }
  }
