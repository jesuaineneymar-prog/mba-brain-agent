...
function ProspectingTab() {
  ...
  const runProspect = async () => {
    ...
    try {
      ...
      if (d.profiles) { setResults(d.profiles); setSelected(new Set()); }
  ...
  return (
    <div style={{ padding:16, overflowY:'auto', height:'100%' }}>
      ...
      {results.length > 0 && <Panel style={{ marginBottom:14 }}><STitle>Resultados da prospeccao ({results.length} perfis)</STitle><div style={{ color:P.textSec, fontSize:12, marginBottom:10 }}>{results.length} perfis encontrados. A guardar na base de dados...</div></Panel>}
      <Panel>
